...
const MagicButton = ({title, icon, position, handleClick, otherClasses} : {title: string, icon: React.ReactNode, position: string, handleClick?: () => void, otherClasses?: string;}) => {
  return (
    <button className="w-full rounded-lg relative inline-flex h-12 overflow-hidden rounded-full p-[1px] focus:outline-none md:w-60 md:mt-10" onClick={handleClick} >
